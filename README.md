...
1. 두 제출 파일의 검사 결과를 확인합니다. 검사는 **10명 선발 1회 → 선발 9명 타순 결정 1회** 순서로 실행합니다.
...
- 함수는 이닝 시작에 팀당 1회, 총 2회 호출되며 `{"defense": [10명], "offense": [9명]}` dict를 반환합니다. 공수교대 때 다시 뽑지 않습니다.
- `defense`는 `[내야수×4, 외야수×3, 포수, DH, 투수]` 순서의 10명, `offense`는 그중 투수를 제외한 같은 9명(DH 포함)의 타순입니다.
- 양 팀 명단을 이닝 시작에 동시에 정하므로 이번 이닝 상대 투수는 아직 알 수 없습니다(직전 이닝 기준으로만 제공). 공수교대 때 선수·투수를 교체하지 않으며, 교체는 다음 이닝 시작에 가능합니다.
- 체력은 **한 경기 안에서 회복되지 않으며**, 다음 경기는 새 체력 상태로 시작합니다.
- 기본 제한시간은 함수 호출당 10초입니다. 경기 중 실패하거나 규칙을 위반한 반환은 직전 이닝 명단 또는 기본 명단으로 대체됩니다.
