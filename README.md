...
- 함수는 이닝 시작에 총 4회(두 학생 각각 10명 선발·타순 결정) 호출됩니다. 공수교대 때 다시 뽑지 않습니다.
- 공격은 타자 9명, 수비는 `[내야수×4, 외야수×3, 포수, DH, 투수]` 순서의 10명을 반환합니다.
- **이닝 시작에 투수 포함 10명을 선발**합니다. 공격은 그중 투수를 제외한 같은 9명(DH 포함)의 타순만 정하며, 공수교대 때 선수·투수를 교체하지 않습니다. 교체는 다음 이닝 시작에 가능합니다.
- 체력은 **한 경기 안에서 회복되지 않으며**, 다음 경기는 새 체력 상태로 시작합니다.
- 기본 제한시간은 함수 호출당 10초입니다. 경기 중 실패한 호출은 직전 유효 명단 또는 기본 명단으로 대체됩니다. 공격 타순 대체도 이번 이닝 선발 9명 안에서만 합니다.
